    ...
    <main className="relative px-3 md:px-6 pt-24 md:pt-28 pb-12">
      <section className="relative mx-auto max-w-7xl mb-10 md:mb-14 px-2">
        ...
        <h1 className="font-display mt-5 text-5xl md:text-7xl leading-[0.95] text-ink text-balance">
          The names we
          <br />
          <span className="grad-text">remember.</span>
        </h1>
      ...
      <section className="relative mx-auto max-w-7xl mb-12 md:mb-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <BigStat label="Total selections" value="142" sub="Since 2018" />
          <BigStat label="IIT seats" value="38" sub="JEE Advanced" highlight />
          <BigStat label="NEET MBBS" value="74" sub="Govt + Private" />
          <BigStat label="100%ile-ers" value="6" sub="JEE Main" />
        </div>
      </section>

      {/* Toppers — vertical stacked cards inspired by 3-phone layouts */}
      <section className="relative mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
          {TOPPERS.map((t, i) => (
            <TopperCard key={t.name} t={t} index={i} />
          ))}
        </div>
      </section>
      ...
      <section className="relative mx-auto max-w-7xl mt-24 md:mt-32">
        <div
          className="skeuo-card-lifted p-10 md:p-14 text-center relative overflow-hidden"
          style={{
            backgroundImage:
...
function BigStat({
  ...
  return (
    <div
      className={`${highlight ? "skeuo-card-lifted" : "skeuo-card"} p-6 md:p-7 relative overflow-hidden`}
      style={
        highlight
          ? {
              backgroundImage:
    ...
    >
      ...
      <div className={`font-display text-5xl md:text-6xl leading-none ${highlight ? "grad-text" : "text-ink"}`}>
        {value}
      </div>
...
function TopperCard({ t, index }: { t: (typeof TOPPERS)[number]; index: number }) {
  return (
    <article className="skeuo-card p-7 md:p-8 relative">
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <div className="mt-4 skeuo-pressed px-5 py-3 inline-flex items-center gap-3">
          ...
          <span className="font-display text-xl grad-text leading-none">{t.rank}</span>
        </div>
